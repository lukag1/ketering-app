...
public class IstorijaNarudzbinaServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            List<Narudzbina> narudzbine = narudzbinaDAO.getNarudzbineByKorisnikId(korisnik.getId());
            request.setAttribute("narudzbine", narudzbine);
            request.getRequestDispatcher("istorija-narudzbina.jsp").forward(request, response);
        } catch (SQLException e) {
            throw new ServletException("Greška pri dohvatanju istorije narudžbina", e);
        }
    }
}
